...
### PR #40 corrective-phase execution evidence (append-only)

- **C1+C2 (policy binding + review-state validity), commit 968e01b** (7 files, +302/−22): `createSessionFromEngineText(text, policyId)` + `startReviewSession` passes `job.policyId`; typed `PolicyError` surfaced in App's review-step error path; `setPolicy` resets review completeness and both output availabilities on a REAL change (unchanged policy = exact no-op); `useJobSession.updatePolicy` drops the now-invalid ReviewSession. Oracles: strict vs standard differ materially (hospital → 'Centro Sanitario' under strict), external-ai/longitudinal-research fail closed with the typed PolicyError (no silent standard fallback), App flow proves a completed review cannot be relabelled and an unchanged policy is a no-op. Native review: lineage **review-2f39fb0952374a35** APPROVED + acknowledged/burned (consumed revision sha256:4e69d844edaf29210973a3cd5ff69d986cae8f55f47e92bcaf5502e5c6586a20). No findings.
- **C3 (secure session identity), commit 20ed3cf** (2 files, +102/−2): `sessionId` now generated via Web Crypto `randomUUID`; typed `SessionIdError` (`secure-session-id-unavailable`) fails closed when the primitive is unavailable — no `Math.random` fallback. Oracles: canonical UUID shape, distinct ids per call, ReviewSession stable-ID namespacing preserved, and both unavailability paths (missing `crypto`, missing `randomUUID`). Native review: lineage **review-98bba01cdebb0059** APPROVED + acknowledged/burned (consumed revision sha256:f74c3b50e24cb552ae8ae8aaee5ecc8a549c49ce3a715ccada3674688b370e98). Advisory R3-001 (SUGGESTION, registry-engine.test.ts:616, informational).
- **C4 (debt reconciliation, docs-only)**: new OPEN **ARCH-012** in docs/DEBT_REGISTER.md (composed V4 engine omits legacy `scoring.entidadesDescartadas`/`descartadas` detail; no current V4 consumer; resolution requires an explicit recognizer/domain contract; deferred owner T14 #18). ARCH-004 stays DONE and now points to ARCH-012 instead of embedding the residual.
- Note: the `registry-engine.ts` header comment already declares the omitted `scoring.descartadas` detail as follow-up debt recorded in this feature document; ARCH-012 is the durable register owner. No code comment change was needed (documentation-only C4).

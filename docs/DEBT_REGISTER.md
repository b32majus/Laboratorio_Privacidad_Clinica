...
| ARCH-012 | P2 | Core/Scoring | El motor V4 compuesto (`app-v4/src/engine/registry-engine.ts`) NO reproduce el detalle legacy `scoring.entidadesDescartadas` / `scoring.descartadas` (candidatos por debajo del umbral de confianza y su razón de scoring): el contrato de observación de reconocedores (WU1/T11 #15) transporta deliberadamente solo observaciones ya filtradas y no internals de scoring. Residual descubierto y declarado durante T11 #15 y reafirmado por la auditoría de promoción de PR #40 (C4). Alcance truthful: (a) hoy ningún consumidor V4 depende de esa contabilidad legacy — `js/domain/from-processor.js` lee solo `entities` + `sessionId`, y las superficies V4 (ReviewSession, Safe Output, Confidential Audit) no la usan; (b) la resolución futura exige exponer candidatos descartados/bajo umbral mediante un contrato explícito de reconocedor/dominio (no fabricarlos en el motor); (c) propietario/destino diferido: **T14 #18 — Low-confidence candidate review queue**, que debe hacer visibles esos candidatos como estado de dominio/revisión. No implementar T14 en T11/PR #40. Estado de ARCH-004: permanece DONE (la separación recognizer/operator/policy sí está materialmente implementada); este residual es deuda independiente. | OPEN |

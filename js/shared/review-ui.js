...
function renderMainText(result) {
  ...
  const html = SafeRender.buildHighlightedHtmlFromOriginal(result, (entity, originalIndex, escapers) => {
    return createEntitySpan(entity, originalIndex, escapers);
  });
  container.innerHTML = SafeRender.toParagraphsHtml(html);
...
function createEntitySpan(entity, index, escapers) {
  ...
  const escapeAttr = escapers?.escapeAttr || SafeRender.escapeAttr;
  const escapeHtml = escapers?.escapeHtml || SafeRender.escapeHtml;
  return `<span id="${escapeAttr(uniqueId)}" class="${escapeAttr(classes)}" data-type="${escapeAttr(entity.type)}" data-original="${escapeAttr(entity.original || entity.text)}">${escapeHtml(entity.transformed)}</span>`;
}
...
function renderSidePanel(result) {
  ...
  result.entities.forEach((entity, index) => {
    ...
    const safeTransformed = SafeRender.escapeHtml(entity.transformed || '');
    const safeOriginal = SafeRender.escapeHtml(entity.original || entity.text || '');

    card.innerHTML = `
      <div class="flex items-center gap-2 mb-1">
        <span class="material-symbols-outlined ${colorClass} text-lg">${iconName}</span>
        <p class="text-stone-900 dark:text-stone-100 font-medium text-sm truncate">${safeTransformed}</p>
      ...
      <p class="text-stone-500 dark:text-stone-400 text-xs">Original: <span class="font-semibold">${safeOriginal}</span></p>
    `;
